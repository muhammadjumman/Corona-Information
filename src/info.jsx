...
const Info = () => {
  ...
  useEffect(() => {
    fetch(
      `https://covid-193.p.rapidapi.com/statistics?country=${theCountry}`,
      options
    )
      .then((response) => response.json())
  ...
  return (
    <Grid mt={20} mb={20}>
      ...
      >
        <Card>
          <CardHeader textTransform={"uppercase"} pt={2} pb={2}>
            Covid Info
          </CardHeader>
          <Divider />
          <CardBody>
            <FormControl>
              ...
              <Select
                ...
                onChange={(e) => setTheCountry(e.target.value)}
              >
                {country &&
                  country.map((e) => (
            ...
            {toggle && (
              <>
                ...
                />
                <Death
                  newCase={info.deaths.new}
                  perMillion={info.deaths["1M_pop"]}
                  total={info.deaths.total}
                />
                <Tests
                  perMillion={info.tests["1M_pop"]}
